-- =============================================================================
-- 05_install_test.sql  —  smoke test post-installazione quicksql MLE
...
BEGIN
    DBMS_OUTPUT.put_line(CHR(10) || '--- Smoke test quicksql_pkg ---' || CHR(10));

    -- Test 1: versione bundle
    l_result := quicksql_pkg.version();
    check_test('version(): stringa non nulla', l_result IS NOT NULL, l_result);

    -- Test 2: tabella semplice
    l_result := quicksql_pkg.to_ddl('employees' || CHR(10) || '  name /nn');
    ...
    l_errors := quicksql_pkg.validate('orders' || CHR(10) || '  amount num');
    check_test('validate: input valido ritorna []', l_errors = '[]');

    -- Test 6: struttura errore ha from.line (non line diretto)
    l_errors := quicksql_pkg.validate('employees' || CHR(10) || '  - bad');
    DECLARE
        l_line NUMBER;
    BEGIN
    ...
    BEGIN
        l_result := quicksql_pkg.to_ddl('simple_tbl', NULL);
