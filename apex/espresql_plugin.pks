create or replace package espresql_plugin
    authid definer
as
    /*
    ** EspreSQL — APEX Region Plugin render package
    **
    ** Render function for the "IT.RADICLE.ESPRESQL" region plug-in.
    ** The plug-in renders EspreSQL directly into the APEX page (no iframe).
    ** HTML, CSS, and JS are served from static files uploaded to the application.
    **
    ** CSS files loaded by the render function (in order):
    **   {base}web/app.css          — EspreSQL UI styles
    ...
    **   {base}web/app_all.js       — bundled EspreSQL app (run: npm run build:web)
    **
    ...
    **   Attribute 04 — Files base URL  : path prefix where EspreSQL static files
    **                                    are uploaded, ending with "/".
    **                                    Supports #APP_IMAGES# and #WORKSPACE_IMAGES#
    **                                    substitutions.
    **                                    Example: #APP_IMAGES#espresql/v2/
    */
