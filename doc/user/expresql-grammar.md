...
| `/bridge` | Marks a table as an N:M associative table (`user_role(user_id, role_id)`). Requires exactly 2 `/fk` columns (warning otherwise) — the first declared is "left", the second "right". Additive, not a replacement: `create_rec`/`update_rec`/`delete_rec` and `ins`/`upd`/`del` remain fully generated. Adds, alongside them: `grant_<label>`/`revoke_<label>`/`has_<label>`/`list_<label>` in DAL (`grant_row`/`revoke_row`/`has_row`/`list_row`, unlabelled), SVC, `_app` (no `list_<label>` — a multi-row cursor doesn't fit flat OUT parameters), and `_rst` (`grant`/`revoke` read both keys from `:body_text`; `has`/`list` use `:p_<col>` binds like `get_by_<col>`; `list` returns a JSON array like `get_all`). `<label>` is the right column with a trailing `_id` stripped (`role_id` → `grant_role`, `revoke_role`, `has_role`, `list_role`) — falls back to the bare column name otherwise. `grant_<label>` is idempotent: granting an already-granted pair returns the existing row's id instead of raising `[DUPLICATE]`. Also adds `alter table ... add constraint ..._uk_bridge unique (left, right)`, so a concurrent duplicate grant is still caught at the database level, not just by grant's own check-first logic. HKS gains `before_grant`/`after_grant`/`before_revoke`/`after_revoke`, alongside (not instead of) the standard hooks. | Oracle only |
| `/businesskey <col>` | Only meaningful together with `/versioned` (warning otherwise); `<col>` must be a column already declared on the table (warning otherwise). Adds SCD2 business-key navigation on top of `/versioned`'s insert-only version chain: DAL/SVC `get_current(<col>)` (the row where `is_current = 1` for that key), `get_as_of(<col>, date)` (the version whose `valid_from`/`valid_to` range covers that instant), `history(<col>)` (a ref cursor of every version for that key, oldest first) — all read from `<table>_rls`, like every other read path. SVC/`_app`/`_rst` also get `change_rec` — closes the current version (`close_version`) and opens the next one (`create_rec`) as a single call, instead of the caller orchestrating both. Also adds `create unique index <table>_<col>_cur_uk on <table> (case when is_current = 1 then <col> end)` so at most one row per key can ever be current — the same guarantee the TAPI's own orchestration relies on, enforced at the database level too. `_app` has no `history` (a multi-row result doesn't fit flat OUT parameters); `_rst`'s `history` returns a JSON array, same shape as `get_all`. **On `db: 26ai`+ only** (26ai first shipped `CREATE ASSERTION`, a SQL-standard multi-row invariant checked immediately on every INSERT/UPDATE — see [`db`](#db)), also adds `create assertion <table>_<col>_no_overlap ...` so no two version rows for the same key can ever have overlapping `[valid_from, <close-column>)` windows — `cur_uk` alone only rules out two rows being *concurrently open* for the same key, not a raw INSERT (outside `change_rec`'s disciplined close-then-open order) landing a window that overlaps an existing closed or open one. Touching is allowed (one window's end may equal another's start); an open window (`<close-column> IS NULL`) is treated as extending to `DATE '9999-12-31'`. Additive to `cur_uk`, never a replacement — on any older `db`, only `cur_uk` is generated, unchanged from before. | Oracle only |
...
| `/immutable` | *(21c+)* Append-only table. A BEFORE UPDATE OR DELETE trigger raises an application error; on 23ai+ also uses the native `CREATE IMMUTABLE TABLE` clause (`NO DROP UNTIL 0 DAYS IDLE`, `NO DELETE UNTIL 16 DAYS AFTER INSERT`) for a database-enforced WORM guarantee. On a layered `/api` table, narrows the TAPI the same way `/versioned` does: no `update_row`/`delete_row` (DAL), `update_rec`/`delete_rec` (SVC), `upd`/`del` (`_app`/`_rst`), or `before_update`/`after_update`/`before_delete`/`after_delete` (HKS) are generated at all — only `get`/`ins` and their hooks. Unlike `/versioned`, there is no replacement write operation (no `close`): an immutable table's TAPI is insert-and-read-only. Cannot be combined with `/versioned` on the same table (contradictory: `/immutable` blocks the `valid_to` closure `/versioned` requires). | Oracle only |
...
| `/versioned [valid_to_col]` | Versioned insert-only-per-version table (freeze-by-reference). Adds `valid_from timestamp default systimestamp not null` and `valid_to timestamp` (or a custom column name) if not already declared, plus a virtual `is_current` column. Generates a `<table>_current` view (`where is_current = 1`). On a layered `/api` table, adds `close_row`/`close_version`/`close` (DAL/SVC/`_app`+`_rst`) **alongside** the normal `update_row`/`delete_row`/`update_rec`/`delete_rec`/`upd`/`del`/etc. — not a replacement: a version row can be freely corrected (any column, including `<vtCol>` itself — that's how `close` itself works under the hood) or deleted, but **only while it has never been closed**; once `<vtCol>` is set, that row is permanent history and every one of update/delete/re-close is rejected (`ORA-20057 [VERSIONED] ...: this version row is already closed`). This is enforced in the DAL itself (the `and <vtCol> is null` clause on `close_row`/`update_row`/`delete_row`'s WHERE, plus a disambiguated error on 0-rows-affected) — business rules belong in the TAPI, not a trigger. **Without a layered `/api`** on the table (no TAPI exists to carry the guard), a BEFORE UPDATE OR DELETE trigger is generated instead — the only enforcement available when there is no PL/SQL API layer to put it in; it blocks all DELETEs, blocks any UPDATE that is not a plain `valid_to` closure, and verifies no other business column changed in the same statement. Bypass protection against direct table access (SQL run outside the TAPI) is a `GRANT`/`REVOKE` concern in either case — deny `INSERT`/`UPDATE`/`DELETE` on the base table and expose only the `_rls` view plus the packages (SmartDB pattern) — never something a trigger or the TAPI itself can guarantee on its own. Distinct from `/history` (a shadow table for CDC/audit); here the FK referencing a specific row always points to the exact historical version used at that moment. Cannot be combined with `/immutable` on the same table (contradictory: `/immutable` blocks the `valid_to` closure this directive requires). | Oracle only |
...
[Sales-Product-Customer Example](../../test/star/sales_product_customers.esql)
...
- **`invoices_rls`** — a view, `select * from sec_pkg.secured_by_dimension(invoices)`, generated once, ahead of every other package for that table (so everything downstream, including the absorbed private DML on tiers without `_dal`, compiles against an existing view).
- **Read-path redirection** — `get_by_id`, `lock_by_id`, `get_all` and `get_by_<unique>` (in `_dal`, or the absorbed private DML on tiers without `_dal`) select from `invoices_rls` instead of `invoices`, rather than re-deriving their own WHERE-clause filter. One filter (the `sec_pkg.secured_by_dimension` macro you provide, the same one any APEX region/report reads through), used identically by every reader of that table — not duplicated per consumer. An out-of-scope row never leaves the read path — same `NO_DATA_FOUND` path as a genuinely missing id, so a caller cannot distinguish "does not exist" from "not in your scope".
- **`companies` itself** gets no `chk_rls` (it has no column configured in the map — `chk_rls` is only ever generated for tables that actually carry a configured dimension column, never as an empty stub) but it **does** still get a `companies_rls` view: `select * from companies`, an unfiltered passthrough. Every table gets a `<table>_rls` view unconditionally, scoped or not — the presentation layer (APEX regions, a future REST surface, this table's own DAL) must never read a table directly, only a view, even when there's nothing to filter. `companies_dal.get_by_id`/`get_all`/etc. read from `companies_rls`, exactly like `invoices_dal` reads from `invoices_rls`, just with no filter applied.

Write paths (`insert_row`/`update_row`/`delete_row`) never read from or reference `<table>_rls` — only the explicit `chk_rls` check governs writes, matching the "raise, never silently no-op" behavior above.
...
A table can carry more than one dimension column (e.g. `{ company_id: "COMPANY", region_id: "REGION" }` on the same table): `chk_rls` checks each one on write. On read, there is still only ONE `<table>_rls` view — ExpreSQL never emits a per-dimension predicate; `secured_by_dimension` itself is expected to introspect the table's real columns and AND every dimension it finds, so a table with several configured columns needs no different treatment here than one with a single column.
...
See the [Car Racing Example](../../test/DV/car_racing/1.esql).
