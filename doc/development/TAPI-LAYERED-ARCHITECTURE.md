...
- **Grammar reference**: [doc/user/expresql-grammar.md §api](../user/expresql-grammar.md#api)

---

## 1. Critical Analysis of the Current Generated TAPI

The TAPI produced by ExpreSQL (`/api` directive) is a functional scaffold, not a production-ready API. Before defining the target architecture, the defects must be understood explicitly.
...
## 2. ExpreSQL Default vs. Layered TAPI — At a Glance

| Capability | ExpreSQL Default (`/api`) | Layered TAPI (this spec) |
|---|---|---|
...
ExpreSQL generates a **single SQL block** containing all packages in compilation order. It does not manage individual files. The hooks body is included in this output as a no-op stub.
...
```
First generation:
  1. Run ExpreSQL → copy the full output into your SQL client
...
Subsequent regenerations (e.g. new column added):
  1. Run ExpreSQL → new output contains updated spec + refreshed no-op body
  ...
  3. Do NOT re-execute the hks body block from ExpreSQL output
...
ExpreSQL includes this block in the generated output. Execute it once to install the stub. Save it separately and replace the `NULL` stubs with custom logic — do not re-execute this block from subsequent ExpreSQL output.
...
-- and manage it independently from the ExpreSQL output.
...
## 11. ExpreSQL Generator Integration
...
**Step 4** — Move existing validation logic into the hooks body. Execute the no-op stub from ExpreSQL output once, save it as a developer-owned file, then replace the stubs with the custom logic ported from the old package.
