# QuickSQL — Composite / Business Primary Keys
...
This document specifies how QuickSQL handles tables whose primary key is a **business key** rather than a surrogate key — i.e. one or more columns that exist in the domain (e.g. `country_code`, `order_no + line_no`) rather than a system-generated `id`.
...
- **Deferrable PK constraints** — not currently modelled in QuickSQL.
