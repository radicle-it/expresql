# QuickSQL Interactive — UI/UX Improvement Specification
...
The current QuickSQL interactive page is functional and visually coherent (Oracle Redwood palette, dark/light themes, syntax highlighting). However, several areas reduce usability and visual clarity:
...
- The product identity (logo + title) is functional but lacks a distinctive QuickSQL mark.
...
```
[Radicle logo] | QuickSQL — write shorthand…  [spacer]  Examples▾  ⚙ Settings  ? Help  ☀  radicle.it
```
...
| Issue | Detail |
|---|---|
| No QuickSQL-specific mark | The product is identified only via text; Radicle logo + text title lacks a dedicated icon |
...
<details class="hdw-sec">
    ...
    <div class="hdw-content">
        ...
        <p class="hdw-note" style="margin-top:6px"><a href="doc/user/quick-sql-grammar.md#settings" target="_blank">→ Full settings reference (all keys and values)</a></p>
    </div>
</details>
```
...
#### P2 — QuickSQL wordmark clarity

The current header reads: `[Radicle logo] | QuickSQL`. The product is "Radicle QuickSQL" but the two components are visually separate. Consider one of:

1. **Layered title**: Keep current layout but increase "QuickSQL" from 16 px to 18 px and make the subtitle a proper second line (using flex-direction: column) rather than an inline span.
2. **Combined mark**: Create a dedicated QuickSQL logo that incorporates both the Radicle brand and the SQL identity — e.g., the Radicle symbol + "QuickSQL" in a single SVG asset.
...
| 18 | Logo | Increase QuickSQL title size / subtitle legibility | P3 | Trivial |
...
- Replacing the QuickSQL parsing engine or output format.
