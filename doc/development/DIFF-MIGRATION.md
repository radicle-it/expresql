# ExpreSQL — Diff / Migration Generator (EX-10)
...
`toDiff()` has no visibility into the live database. If a DBA manually created an index, added a constraint, or altered a column outside of ExpreSQL, the generated script may fail with object-already-exists or integrity-violation errors.
...
Shadow-state objects (created manually outside ExpreSQL) cause ORA-00955 / ORA-02261 on add operations. For Oracle < 23c, creation statements that cannot use `CREATE OR REPLACE` are wrapped in PL/SQL exception handlers:
...
```sql
-- ============================================================
-- ExpreSQL Migration Script — v0.4
...
- **View invalidation**: views defined outside ExpreSQL that reference dropped tables are marked `INVALID` by Oracle. The diff only drops views explicitly defined in `oldQsql`.
