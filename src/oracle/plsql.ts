...
`;
    ...
    private _generateDalSpec(node: IDdlNode): string {
        ...
        const vtCol         = (String(node.getOptionValue('versioned') ?? '').trim().replace(/^=/, '') || 'valid_to').toLowerCase();
     ...
     * generated once here instead of by hand in a separate file: get_by_id/lock_by_id/
     * get_all below read from it directly, replacing the WHERE-clause injection this
     * function used to require — one filter, defined once (secured_by_dimension already
     * generalizes the check via real column introspection, see 02-anagrafiche-
     * sicurezza.rls.sql), used identically by every _rls consumer (APEX regions, and now
     * this table's own DAL). Detail tables without their own dimension column (scope
     * inherited via a JOIN to a header table) are NOT covered here — that _rls view, like
     * their write-side scope check, stays a hand-written exception (see docs/architettura/
     * 04-controllo-accessi-sistematico.md).
     */
    ...
    private _generateDalBody(node: IDdlNode): string {
        ...
        // delete_row, where the explicit chk_rls check in _hks stays authoritative,
        // matching the already-verified RLS1-4 behavior of this project: an
        // out-of-scope write raises, it never silently no-ops.
        ...
        // shared with every other _rls consumer, instead of re-deriving the same WHERE
        // condition here. dimConds itself is no longer used to build a condition; only
        // its presence (via dimSource below) decides which relation to read from.
        const dimSource = hasDimScope ? `${tbl}_rls` : tbl;

        // get_by_id — NO_DATA_FOUND raises c_err_not_found; tenant/dimension-scoped when active.
        ...
        // For /versioned tables, close_row replaces update_row + delete_row (narrow TAPI).
        // When tenantid is active, both operations use tenant_ctx.get_id in the WHERE clause so they
    ...
    private _generateHksSpec(node: IDdlNode): string {
        ...
        // selective by design, see 04-controllo-accessi-sistematico.md §5).
    ...
    private _generateSvcBody(node: IDdlNode): string {
        ...
        r += `${tab}${tab}${hk}.chk_rbac(p_operation => 'insert', p_row => l_row);\n`;
        if (this._dimensionScopeColumns(node).length > 0) r += `${tab}${tab}${hk}.chk_rls(p_row => l_row);\n`;
        ...
        if (isVersioned) {
            ...
            r += `${tab}${tab}${hk}.chk_rbac(p_operation => 'close', p_row => l_row);\n`;
            if (this._dimensionScopeColumns(node).length > 0) r += `${tab}${tab}${hk}.chk_rls(p_row => l_row);\n`;
            r += `${tab}${tab}${hk}.validate(p_operation => 'close', p_row => l_row);\n`;
            r += `${tab}${tab}${hk}.before_close(p_row => l_row);\n`;
            r += `${tab}${tab}${dal}.close_row(p_id => p_id, p_${vtCol} => l_row.${vtCol}, p_row => l_row);\n`;
            r += `${tab}${tab}${hk}.after_close(p_row => l_row);\n`;
            ...
            for (const { name } of paramCols)
                r += `${tab}${tab}l_row.${name} := p_rec.${name};\n`;
            if (hasVer) r += `${tab}${tab}l_row.row_version := p_row_version;\n`;
            r += `${tab}${tab}${hk}.chk_rbac(p_operation => 'update', p_row => l_row);\n`;
            if (this._dimensionScopeColumns(node).length > 0) r += `${tab}${tab}${hk}.chk_rls(p_row => l_row);\n`;
            ...
            // delete_rec — the row is always fetched first (get_by_id, itself scoped when a
            // dimension column is configured: an out-of-scope id raises NOT_FOUND right here,
            // before anything else runs) so that validate('delete', ...) can finally run on
            // delete too — until now the only operation validate() never covered. chk_rbac/
            // chk_rls run first, in front of validate, same ordering as insert/update/close.
            r += `${tab}procedure delete_rec (p_id in ${tbl}.${pkNm}%type) is\n`;
            r += `${tab}${tab}l_row ${tbl}%rowtype;\n`;
            r += `${tab}begin\n`;
            r += `${tab}${tab}l_row := ${dal}.get_by_id(p_id => p_id);\n`;
            r += `${tab}${tab}${hk}.chk_rbac(p_operation => 'delete', p_row => l_row);\n`;
            if (this._dimensionScopeColumns(node).length > 0) r += `${tab}${tab}${hk}.chk_rls(p_row => l_row);\n`;
            r += `${tab}${tab}${hk}.validate(p_operation => 'delete', p_row => l_row);\n`;
    ...
    private _generateAppSpec(node: IDdlNode): string {
        ...
        if (isVersioned) {
            ...
            if (hasVer) closeLines.push(`${tab}${tab}p_row_version  in out ${tbl}.row_version%type`);
            r += closeLines.join(',\n') + `\n${tab});\n\n`;
        } else {
    ...
    private _generateAppBody(node: IDdlNode): string {
        ...
        if (isVersioned) {
            ...
            const closeLines: string[] = [];
            closeLines.push(`${tab}${tab}p_id           in     ${tbl}.${pkNm}%type`);
            closeLines.push(`${tab}${tab}p_${vtCol.padEnd(appPadWidthBody)} in     ${tbl}.${vtCol}%type default systimestamp`);
            if (hasVer) closeLines.push(`${tab}${tab}p_row_version  in out ${tbl}.row_version%type`);
            r += closeLines.join(',\n') + `\n${tab}) is\n`;
            r += `${tab}begin\n`;
    ...
    private _generateAuditBody(node: IDdlNode): string {
        ...
        // tenant_id on the audit log row is set by insert_row via the audit log DAL's tenant_ctx.get_id.
    ...
    private _generateRstSpec(node: IDdlNode): string {
        ...
        r += `${tab}-- Bind :p_id to the URL path parameter, :body_text to the request body,\n`;
        r += `${tab}-- and :status to the HTTP response status output bind.\n\n`;
        r += `${tab}procedure get;\n`;
    ...
    private _generateRstBody(node: IDdlNode): string {
        ...
        const jsonObj = (indent: string) =>
            `json_object(\n${indent}${tab}` +
            jsonCols.join(`,\n${indent}${tab}`) +
            `\n${indent}${tab}returning clob\n${indent})`;

        // shared exception handler block
        const exc =
            `${tab}exception\n` +
        ...
        // get — single row by :p_id
        r += `${tab}procedure get is\n`;
        r += `${tab}${tab}l_row ${tbl}%rowtype;\n`;
        r += `${tab}begin\n`;
        r += `${tab}${tab}l_row := ${svc}.get(p_id => :p_id);\n`;
        r += `${tab}${tab}:status := 200;\n`;
        r += `${tab}${tab}htp.p(${jsonObj(tab + tab)});\n`;
        r += exc;
        r += `${tab}end get;\n\n`;

        // get_all — full collection (tenant-scoped via dal.get_all when tenantid active)
        ...
        r += `${tab}${tab}${tab}htp.p(l_sep || ${jsonObj(tab + tab + tab)});\n`;
        r += `${tab}${tab}${tab}l_sep := ',';\n`;
        r += `${tab}${tab}end loop;\n`;
        ...
        if (isVersioned) {
            ...
            // upd — update :p_id from :body_text
            ...
            if (hasVer) {
                ...
                r += `${tab}${tab}${tab}p_id          => :p_id,\n`;
                r += `${tab}${tab}${tab}p_rec         => l_rec,\n`;
                r += `${tab}${tab}${tab}p_row_version => json_value(l_body, '$.row_version' returning ${tbl}.row_version%type)\n`;
                r += `${tab}${tab});\n`;
            } else {
                r += `${tab}${tab}${svc}.update_rec(p_id => :p_id, p_rec => l_rec);\n`;
            }
            r += `${tab}${tab}:status := 200;\n`;
            r += `${tab}${tab}htp.p(json_object('${pkNm}' value :p_id));\n`;
            r += exc;
