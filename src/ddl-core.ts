...
import tree, { createGenerator, createDiffGenerator, DdlNode } from './tree.js';
...
export class espresql implements DdlContext {
    ...
    setOptions(line: string): void {
        ...
        for (const t of src) {
            if (t.type === 'identifier' &&
                ...
                json += '"' + t.value + '"';
            } else if (t.type === 'constant.numeric' && !/^\d+(\.\d+)?$/.test(t.value)) {
                // Alphanumeric token like "19c" — lexed as numeric but must be a string.
                json += '"' + t.value + '"';
            } else {
                json += t.value;
            }
    ...
    private _makeFooter(): string {
        ...
        let out = `-- Generated by Radicle EspreSQL ${this.version()} ${new Date().toLocaleString()}\n\n`;
        out += '/*\n';
    ...
    declare static version:  typeof espresql_version;
...
export function toERD(input: string, options?: unknown): ErdOutput {
    return new espresql(input, options).getERD();
}

export function toDDL(input: string, options?: unknown): string {
    return new espresql(input, options).getDDL();
}

export function toErrors(input: string, options?: unknown): unknown[] {
    return new espresql(input, options).getErrors();
}
...
export function toDiff(
    ...
    const newCtx = new espresql(newQsql, options);
    const oldCtx = new espresql(oldQsql, options);
    return createDiffGenerator(newCtx).compute(oldCtx, newCtx);
}

export function espresql_version(): string {
    return typeof __PACKAGE_VERSION__ === 'undefined' ? 'development' : __PACKAGE_VERSION__;
}

// Assign to static slots for backward-compat (espresql.toDDL etc.)
espresql.toDDL    = toDDL;
espresql.toERD    = toERD;
espresql.toErrors = toErrors;
espresql.toDiff   = toDiff;
espresql.fromJSON = fromJSON;
espresql.version  = espresql_version;
espresql.lexer    = lexer;

export default espresql;
