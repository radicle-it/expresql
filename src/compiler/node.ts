...
// 'constant.numeric' (see lexer.ts, the numeric-literal branch), even when the rest of the token
// isn't numeric at all (e.g. "2WAY", "3WAY", "24H", "1ST"). That produced invalid, unquoted DDL
// for /check lists whose values happen to start with a digit — check (status in (2WAY,3WAY))
// instead of check (status in ('2WAY','3WAY')), ORA-00907. Already-quoted string literals and
// backtick pass-through literals are never tagged 'identifier' or (falsely) 'constant.numeric'
// by the lexer, so they're unaffected by either branch here — only genuinely bare, unquoted
// tokens get quoted, exactly as before.
